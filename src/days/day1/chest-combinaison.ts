...
export const findChestCombinaison = () => {
  ...
  const inputData = getFileContent(
    `${path.dirname(__filename)}/input-day-1.txt`,
    /\r?\n/
...
const computeChestValue = (
  currentChestValue: number,
  valueToApply: string
): number => {
...
const readRotationInstruction = (
  rotationInstruction: string
): { rotation: "L" | "R"; rotationValue: number } => {
  const rotation = rotationInstruction.slice(0, 1);
  const rotationValue = parseInt(
    rotationInstruction.slice(1, rotationInstruction.length)
  );
